...
export default function TransactionModal({ config, onClose }: VariantProps) {
  const getStatusMessage = () => {
    switch (config.txStatus) {
      case 'preparing':
        return 'Preparing transaction payload and verifying settlement details...'
      case 'signing':
        return 'Signature requested. Please review and authorize the transaction call inside your connected wallet...'
      case 'pending':
        return 'Broadcasting wire transaction payload to the settlement network...'
      case 'confirming':
        return 'Transaction dispatched. Waiting for network verification and final settlement...'
      case 'finalizing':
        return 'Settlement confirmed. Updating local ledger records...'
      case 'success':
        return 'Transfer settled! The funds have been transferred successfully.'
      case 'failed':
        return 'Transaction execution failed. The settlement could not be processed.'
      case 'rejected':
        return 'Transaction signing rejected. The authorization request was denied in your wallet.'
      case 'expired':
        return 'Authorization signature request expired. Please try again.'
      case 'timeout':
        return 'Attestation signatures are taking longer than usual to resolve.'
      default:
        return 'Awaiting transaction response...'
    }
  }
